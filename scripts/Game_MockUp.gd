...
func _on_MainMenu_MainMenu_NewGame_pressed():
	...
	Input_Count = 0
	if $MainMenu.Option_Input_DistanceToNest.pressed:
		Input_Count = Input_Count + 1
	if $MainMenu.Option_Input_Rotation.pressed:
		Input_Count = Input_Count + 1
	if $MainMenu.Option_Input_CollisionDetection.pressed:
		Input_Count = Input_Count + 2
	if $MainMenu.Option_Input_TileDetection.pressed:
		Input_Count = Input_Count + 2
